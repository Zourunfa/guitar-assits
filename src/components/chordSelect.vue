...
function touchMove(index: number, e: TouchEvent | number) {
  ...
  _state[keyBarName] =
    resultX < state.minLeft
      ? state.minLeft
      : resultX > state.maxLeft
      ? state.maxLeft
      : resultX;
  let percent =
    (_state[keyBarName] - state.minLeft) /
    (state.maxLeft - state.minLeft + 0.01);
  ...
  <div class="container-chordSelect">
    <div class="key-select">
      ...
      >
        ...
        <div
          :class="
            'key-bar' + (state['keyBarShow' + (i + 1)] ? ' keybar-show' : '')
          "
          :style="{ left: state['keyBar' + (i + 1)] + 'px' }"
        ></div>
      </div>
    </div>
    ...
    <div class="chord-count">
      <div
        :class="{ noselect: true, active: state.type === 3 }"
        @click="chordCountChange(3)"
      >
        三音和弦
      </div>
      <div
        :class="{ noselect: true, active: state.type === 4 }"
        @click="chordCountChange(4)"
      >
        四音和弦
      </div>
    </div>
